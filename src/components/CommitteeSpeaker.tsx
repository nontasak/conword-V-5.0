...
import { X, Play, Square, Settings, Maximize, Minimize, Minimize2, Minus, Crosshair, Trash2, Plus, Search, RotateCcw, Edit2, Check, UserPlus, AlertCircle, ArrowDownToLine, Eraser, Camera, ScanFace, Loader2 } from 'lucide-react';
import { loadFaceModels, extractCenterFace, saveFaceToDB, getFaceDB, deleteFaceFromDB, recognizeFace, exportFaceDB, importFaceDB, clearFaceDB, trackCenterFace, calculateEuclideanDistance, extractColorGrid } from '../services/faceService';
...
export const CommitteeSpeaker: React.FC<AutoSpeakerProps> = ({
  isOpen,
  onClose,
  onInsertSpeaker,
  senatorList,
  ...
  const [faceThreshold, setFaceThreshold] = useState(() => {
    const saved = localStorage.getItem('committeeFaceThreshold');
    return saved ? parseFloat(saved) : 0.40;
  });
  const [useAutoContrast, setUseAutoContrast] = useState(() => {
    const saved = localStorage.getItem('committeeUseAutoContrast');
    return saved ? saved === 'true' : false;
  });
  const [isDebugMode, setIsDebugMode] = useState(() => {
    const saved = localStorage.getItem('committeeIsDebugMode');
    return saved ? saved === 'true' : false;
  });
  const [faceBoxWidth, setFaceBoxWidth] = useState(() => {
    const saved = localStorage.getItem('committeeFaceBoxWidth');
    return saved ? parseFloat(saved) : 22; // Default 22%
  });
  const [faceBoxHeight, setFaceBoxHeight] = useState(() => {
    const saved = localStorage.getItem('committeeFaceBoxHeight');
    return saved ? parseFloat(saved) : 30; // Default 30%
  });
  const [enableOCR, setEnableOCR] = useState(() => {
    const saved = localStorage.getItem('committeeEnableOCR');
    return saved ? saved === 'true' : true; // Default true
  });
  const [enableReId, setEnableReId] = useState(() => {
    const saved = localStorage.getItem('committeeEnableReId');
    return saved ? saved === 'true' : false;
  });
  const [enableSuperRes, setEnableSuperRes] = useState(() => {
    const saved = localStorage.getItem('committeeEnableSuperRes');
    return saved ? saved === 'true' : false;
  });
  const [enableSpatialBias, setEnableSpatialBias] = useState(() => {
    const saved = localStorage.getItem('committeeEnableSpatialBias');
    return saved ? saved === 'true' : false;
  });
  ...
  const [cropBox, setCropBox] = useState(() => {
    const saved = localStorage.getItem('committeeCropBox');
    if (saved) {
      try {
  ...
  const faceBoxWidthRef = useRef(faceBoxWidth);
  const faceBoxHeightRef = useRef(faceBoxHeight);
  ...
  const lastPersonPositionRef = useRef<{ x: number, y: number, name: string } | null>(null);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('committeeUseAutoContrast', useAutoContrast.toString());
  }, [useAutoContrast]);
  ...
  useEffect(() => { 
    ...
    localStorage.setItem('committeeCropBox', JSON.stringify(cropBox));
  }, [cropBox]);
  useEffect(() => { 
  ...
  useEffect(() => { 
    ...
    localStorage.setItem('committeeFaceThreshold', faceThreshold.toString());
  }, [faceThreshold]);
  useEffect(() => { faceBoxWidthRef.current = faceBoxWidth; }, [faceBoxWidth]);
  useEffect(() => { faceBoxHeightRef.current = faceBoxHeight; }, [faceBoxHeight]);

  useEffect(() => {
    localStorage.setItem('committeeEnableReId', enableReId.toString());
  }, [enableReId]);

  useEffect(() => {
    localStorage.setItem('committeeEnableSuperRes', enableSuperRes.toString());
  }, [enableSuperRes]);

  useEffect(() => {
    localStorage.setItem('committeeEnableSpatialBias', enableSpatialBias.toString());
  }, [enableSpatialBias]);

  const refreshIndexedFaces = () => {
    const db = getFaceDB('committeeFaceDB');
    setIndexedFaces(Object.keys(db));
  };

  const exportCommitteeData = () => {
    try {
      const faceDBData = localStorage.getItem('committeeFaceDB');
      ...
      a.href = url;
      a.download = `committee_data_${new Date().toISOString().split('T')[0]}.json`;
  ...
  const importCommitteeData = (file: File) => {
      ...
      try {
        ...
        if (data && typeof data === 'object') {
          ...
          if (data.faceDB && typeof data.faceDB === 'object') {
            localStorage.setItem('committeeFaceDB', JSON.stringify(data.faceDB));
            refreshIndexedFaces();
            hasImportedSomething = true;
          } else if (!data.senatorList && !data.speakerList && Object.keys(data).length > 0) {
            // Fallback for old face DB format
            localStorage.setItem('committeeFaceDB', JSON.stringify(data));
            refreshIndexedFaces();
            hasImportedSomething = true;
          }
  ...
  useEffect(() => {
    localStorage.setItem('committeeIsDebugMode', isDebugMode.toString());
  }, [isDebugMode]);

  useEffect(() => {
    localStorage.setItem('committeeFaceBoxWidth', faceBoxWidth.toString());
  }, [faceBoxWidth]);

  useEffect(() => {
    localStorage.setItem('committeeFaceBoxHeight', faceBoxHeight.toString());
  }, [faceBoxHeight]);

  useEffect(() => {
    localStorage.setItem('committeeEnableOCR', enableOCR.toString());
  }, [enableOCR]);
  ...
  const handleRegisterFace = async (targetName: string) => {
    ...
    setEnrollProgress(0);
    setEnrollStatus('กำลังเตรียมความพร้อม...');
    
    // Allow UI to update
    await new Promise(resolve => setTimeout(resolve, 300));
    
    const collectedData: { descriptor: Float32Array, extraFeatures: any }[] = [];
    const maxFrames = 3; // Reduced for speed
    const maxAttempts = 15; // Faster timeout
    let attempts = 0;
    
    setEnrollStatus('กำลังบันทึกอัตลักษณ์...');
    ...
    while (collectedData.length < maxFrames && attempts < maxAttempts) {
      ...
      try {
        ...
        if (detection) {
          // Check if this descriptor is different enough from what we already collected
          let isNew = true;
          for (const existing of collectedData) {
            const dist = calculateEuclideanDistance(detection.descriptor, existing.descriptor);
            if (dist < 0.05) { // Very lenient similarity check
              isNew = false;
              break;
            }
          }
          ...
          if (isNew) {
            ...
            const progress = Math.round((collectedData.length / maxFrames) * 100);
            setEnrollProgress(progress);
            setEnrollStatus(`เก็บข้อมูล: ${collectedData.length}/${maxFrames}`);
            
            // Visual feedback: show a temporary box
            ...
            setFaceBox({
              ...
              color: '#a855f7' // Purple for enrollment
            });
          }
        }
      } catch (err) {
        console.error("Error during burst capture:", err);
      }
    ...
    try {
      if (collectedData.length > 0) {
        setEnrollStatus('กำลังวิเคราะห์อัตลักษณ์ด้วย AI...');
        ...
        setEnrollStatus('กำลังบันทึกข้อมูล...');
        
        // Save all collected descriptors
        let successCount = 0;
        for (const data of collectedData) {
          const res = saveFaceToDB(targetName, data.descriptor, 'committeeFaceDB', { ...data.extraFeatures, attributes });
          if (res === true || res === 'duplicate') successCount++;
        }
        ...
        if (successCount > 0) {
          ...
          setToast({ show: true, msg: `บันทึกอัตลักษณ์ ${targetName} สำเร็จ (${successCount} เฟรม)${attrMsg}!`, type: 'success' });
          return true;
        }
      } else {
        setToast({ show: true, msg: `ไม่สามารถเก็บข้อมูลอัตลักษณ์ได้เพียงพอ กรุณาลองใหม่ในที่สว่างและมองตรง`, type: 'error' });
        return false;
      }
    } catch (err) {
  ...
  const performFaceTracking = async () => {
    if (!videoRef.current || !isCapturingRef.current || !isFaceApiLoaded || isTrackingFaceRef.current || isProcessingFaceRef.current) return;
    ...
    try {
      ...
      if (box) {
        ...
        setFaceBox(prev => {
          ...
          const targetWidth = (faceBoxWidthRef.current / 100) * vw;
          const targetHeight = (faceBoxHeightRef.current / 100) * vh;
  ...
  const performMetadataExtraction = async () => {
    if (!videoRef.current || !isCapturingRef.current || !isFaceApiLoaded) return;
  ...
  const performFaceRecognition = async () => {
    if (!videoRef.current || !isCapturingRef.current || !isFaceApiLoaded || isProcessingFaceRef.current) return;
      ...
      if (result && result.box) {
        ...
        if (enableSpatialBias) {
          ...
          if (currentMatch === 'Unknown' && bestSeatKey) {
            ...
            // If we have a very strong history for this seat (count > 15)
            // and the face recognition was "close" (distance < threshold * 1.5)
          ...
          if (currentMatch !== 'Unknown' && result.distance < faceThresholdRef.current * 0.9) {
            ...
            if (existing.name === currentMatch) {
              ...
              // If someone else is here, decrease confidence of previous occupant or replace if low
              existing.count -= 2;
              if (existing.count <= 0) {
        ...
        setFaceBox(prev => {
          ...
          const targetWidth = (faceBoxWidthRef.current / 100) * videoW; 
          const targetHeight = (faceBoxHeightRef.current / 100) * videoH;
          
          let newLabel = isUnknown ? 'ไม่รู้จัก' : finalMatch;
  ...
  return (
    <>
      ...
      <motion.div 
        drag={!isMinimized}
        dragMomentum={false}
        className={`fixed ${isMinimized ? 'bottom-4 right-4 w-[420px]' : (isCompact ? 'bottom-4 right-4 w-[380px]' : `top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${!stream ? 'w-[500px]' : 'w-[800px]'} max-w-[95vw]`)} bg-white rounded-xl shadow-2xl z-50 overflow-hidden border border-gray-200 flex flex-col transition-all duration-300`}
      ...
      <div className="bg-blue-600 text-white p-3 flex justify-between items-center cursor-move shrink-0">
        <h3 className="font-bold flex items-center gap-2">
          <Crosshair size={18} />
          {isCompact ? 'เดาชื่อ (กะทัดรัด)' : 'ระบบเดาชื่อผู้พูดกรรมาธิการ (OCR)'}
        </h3>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setIsCompact(!isCompact)} 
            className={`p-1 rounded transition-colors ${isCompact ? 'bg-white/20' : 'hover:bg-blue-700'}`}
            title={isCompact ? "ขยายหน้าจอ" : "โหมดกะทัดรัด"}
          >
            <Minimize2 size={16} />
          </button>
      ...
      <div className={isMinimized ? 'hidden' : `p-4 flex flex-col gap-4 overflow-y-auto flex-1 ${isCompact ? 'max-h-[500px]' : ''}`}>
          {/* Video Preview & Crop Box */}
          <div className={`relative bg-black rounded-lg overflow-hidden flex items-center justify-center ${isCompact ? 'h-[240px]' : 'min-h-[180px]'} shrink-0 border border-gray-200 shadow-inner`}>
            {!stream && (
              <button 
              ...
              <div className="relative inline-block max-w-full max-h-[40vh] group">
                ...
                {faceBox && (
                  ...
                  >
                    {faceBox.label && (
                      <div 
                        className="absolute -bottom-6 left-1/2 -translate-x-1/2 text-white text-[10px] px-2 py-0.5 rounded whitespace-nowrap font-bold shadow-md"
                        style={{ backgroundColor: faceBox.color }}
                      >
                        {faceBox.label}
                      </div>
                    )}
          ...
          {/* Enrollment Progress Bar (Below Video) */}
          {isRegisteringFace && (
            <div className="bg-purple-50 border border-purple-200 rounded-lg p-3 animate-in fade-in slide-in-from-top-2 duration-300 shrink-0">
              <div className="flex items-center gap-4">
                <div className="relative w-12 h-12 shrink-0">
                  <Loader2 className="w-12 h-12 text-purple-500 animate-spin" />
                  <div className="absolute inset-0 flex items-center justify-center text-purple-700 font-bold text-[10px]">
                    {enrollProgress}%
                  </div>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-1">
                    <h3 className="text-purple-900 font-bold text-sm truncate">กำลังบันทึกอัตลักษณ์...</h3>
                    <span className="text-[10px] text-purple-600 font-medium">{enrollStatus}</span>
                  </div>
                  ...
                  <p className="text-[10px] text-purple-500 mt-1">
                    กรุณาขยับอัตลักษณ์ช้าๆ เพื่อให้ระบบเก็บข้อมูลได้หลายมุมมอง
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Controls */}
          <div className={`flex justify-between items-center bg-gray-50 rounded-lg border border-gray-100 ${isCompact ? 'p-1' : 'p-2'}`}>
            <div className="flex gap-2">
              {isCapturing && (
                <button 
                  onClick={stopCapture} 
                  className={`flex items-center gap-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-bold shadow-sm transition-all active:scale-95 ${isCompact ? 'px-2 py-1 text-xs' : 'px-4 py-2 text-sm'}`}
                >
                  <Square size={isCompact ? 12 : 16} fill="currentColor" /> {isCompact ? 'หยุด' : 'หยุดอ่าน'}
                </button>
              )}
            </div>
            <button 
              onClick={() => setIsSettingsOpen(!isSettingsOpen)}
              className={`flex items-center gap-2 rounded-lg font-bold transition-all shadow-sm border ${
                isSettingsOpen 
                  ? 'bg-blue-600 text-white border-blue-600 ring-2 ring-blue-100' 
                  : 'bg-white text-gray-700 hover:bg-gray-50 border-gray-300'
              } active:scale-95 ${isCompact ? 'px-2 py-1 text-xs' : 'px-4 py-2 text-sm'}`}
            >
              <Settings size={isCompact ? 12 : 16} className={isSettingsOpen ? 'animate-spin-slow' : ''} /> 
              {isSettingsOpen ? 'ปิดตั้งค่า' : (isCompact ? 'ตั้งค่า' : 'ตั้งค่ารายชื่อตั้งต้น')}
            </button>
          </div>
          ...
          {stream && (
            <div className="flex flex-col gap-2">
              <button 
                ...
                className="text-[10px] text-blue-600 hover:text-blue-800 underline self-start font-medium"
              >
                {isManualCropOpen ? 'ซ่อนการตั้งค่าตำแหน่งด้วยตัวเลข' : 'ตั้งค่าตำแหน่งด้วยตัวเลข (ขั้นสูง)'}
              </button>
              
              {isManualCropOpen && (
                <div className="grid grid-cols-2 gap-4 bg-gray-50 p-3 rounded border border-gray-200 text-[10px]">
                  <div>
                    <label className="block mb-1 text-gray-600">ตำแหน่ง X: {cropBox.x}%</label>
                    <input type="range" min="0" max="100" value={cropBox.x} onChange={e => setCropBox({...cropBox, x: Number(e.target.value)})} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                  <div>
                    <label className="block mb-1 text-gray-600">ตำแหน่ง Y: {cropBox.y}%</label>
                    <input type="range" min="0" max="100" value={cropBox.y} onChange={e => setCropBox({...cropBox, y: Number(e.target.value)})} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                  <div>
                    <label className="block mb-1 text-gray-600">ความกว้าง: {cropBox.width}%</label>
                    <input type="range" min="5" max="100" value={cropBox.width} onChange={e => setCropBox({...cropBox, width: Number(e.target.value)})} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                  <div>
                    <label className="block mb-1 text-gray-600">ความสูง: {cropBox.height}%</label>
                    <input type="range" min="5" max="100" value={cropBox.height} onChange={e => setCropBox({...cropBox, height: Number(e.target.value)})} className="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-blue-500" />
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Error Message */}
          {errorMsg && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg relative text-sm flex justify-between items-center shadow-sm">
              <div className="flex items-center gap-2">
                <AlertCircle size={18} />
          ...
          {/* Settings Panel */}
          {isSettingsOpen && (
            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 flex flex-col gap-4 shadow-sm">
              {/* Tabs */}
              ...
              {activeTab !== 'faces' && activeTab !== 'settings' && (
                <div className="flex justify-between items-center">
                  <label className="text-sm font-bold text-gray-700">
                    {activeTab === 'senators' ? 'รายชื่อกรรมาธิการ' : 'รายชื่อผู้ชี้แจง'} ({activeTab === 'senators' ? senatorList.length : speakerList.length})
                  </label>
                <div className="flex gap-2">
                  <button 
                    onClick={() => {
                      setConfirmDialog({
                        show: true,
                        message: 'คุณต้องการสลับรายชื่อทั้งหมดระหว่าง "กรรมาธิการ" และ "ผู้ชี้แจง" หรือไม่?',
                        onConfirm: () => {
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="relative flex-1">
                    <input 
                      ...
                      placeholder={`เพิ่มรายชื่อ${activeTab === 'senators' ? 'กรรมาธิการ' : 'ผู้ชี้แจง'}ใหม่...`}
                                ...
                                <button 
                                  onClick={() => {
                                    setConfirmDialog({
                                      ...
                                      onConfirm: () => {
                                        deleteFaceFromDB(faceName, 'committeeFaceDB');
                                        refreshIndexedFaces();
                                      }
                                    });
                                  }}
        ...
        <div className="min-h-[76px] flex flex-col justify-center">
          {predictions.length > 0 ? (
            ...
            <div className="flex gap-1">
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="flex gap-1 shrink-0">
                          ...
                          {isSenator && <span className="px-1.5 py-0.5 rounded text-[10px] bg-[#800000] text-white font-bold">กรรมาธิการ</span>}
                          {!isSenator && isSpeaker && <span className="px-1.5 py-0.5 rounded text-[10px] bg-blue-100 text-blue-800 border border-blue-200 font-bold">ผู้ชี้แจง</span>}
              ...
              {!isMinimized && (
              <button 
                onClick={() => handleRegisterFace(formatSpeakerName(predictions[0]))}
                disabled={isRegisteringFace || !isFaceApiLoaded}
